...
description: "An R learing lab exploring the Palmer Penguines data set.  In this lab, you will build and demonstrate proficiency in working with data.frames and plotting with base R syntax."
---
...
One of the best ways to understand what's in a data set is to look the structure summary.  Go ahead and display the structure summary for `pengins`.
...
Wow!  Lots of duplicates!  Can you display a vector that eliminates all of the duplicates and just has one value for each species (a "unique vector").  Do the same for the `island` column and the `year` column.
...
If that's still not clear, let's look at such a table. 
...
If at least one Adelie penguin was observed on on Biscoe Island, then there will be *at least one record* in the `penguins` data set that includes the combination "Adelie" and "Biscoe", right?  

If there were never a Chinstrap penguin observed on Dream Island, there would not be any record in the data set with the combination of "Chinstrap" and "Dream", right?

So think about this next statement carefully until you really understand *why* it's true...

_All we have to do is: 1) select the `island` and `species` columns from the `penguins` data; and 2) find the unique records cross those columns!_

Remember that when you select a subset of columns from a data.frame, the result in also a data.frame -- with fewer columns!

You could try to write a function to find the unique records in a data.frame, but this is the type task where you should ask yourself "Is there already a function that does that?"
