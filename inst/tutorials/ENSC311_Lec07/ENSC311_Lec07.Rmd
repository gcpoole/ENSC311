...
The integer inside the brackets in the index number (position in the vector) of the first value in the row.  
...
Until now, when we have referred to "names" in R, we've talked about names that are associated with "objects" in R.  (When I say "object", I'm referring to things like vectors and functions.)  As you know, these object names are created and bound to an object using the assignment `<-` operator.

For instance, I can create and refer to the object name `random_vector` like this:
...
We call a vector with value names a "named vector."
...
The most important use of value names is indexing.  If you name the values, it's often times easier to work with the data.  For instance, 
