...
```{r vector_using_c, exercise=TRUE}

```

```{r vector_using_c-solution}
my_vector <- c(5, 6, 7, 8, 9)
my_vector
```

```{r vector_using_c-code-check}
grade_code()
```
...
```{r vector_using_colon_1, exercise=TRUE}

```

```{r vector_using_colon_1-solution}
-10:10
```

```{r vector_using_colon_1-code-check}
grade_code()
```

What happens when you reverse the numbers?

```{r vector_using_colon_2, exercise=TRUE}

```

```{r vector_using_colon_2-solution}
10:-10
```

```{r vector_using_colon_2-code-check}
grade_code()
```
...
```{r vector_using_seq_1, exercise=TRUE}

```

```{r vector_using_seq_1-solution}
seq(30, 40, 2)
```

```{r vector_using_seq_1-code-check}
grade_code()
```
...
```{r vector_using_seq_2, exercise=TRUE, exercise.reveal_solution=FALSE}
seq(40, 30, 2)
```

```{r vector_using_seq_2-solution}
seq(40, 30, -2)
```

```{r vector_using_seq_2-code-check}
grade_code()
```
...
```{r pass_explicit_parameters, exercise=TRUE}
seq(by = ___, to = ___, from = ___)
```

```{r pass_explicit_parameters-solution}
seq(by = 2, to = 10, from = 0)
```

```{r pass_explicit_parameters-code-check}
grade_code()
```
...
```{r pass_mixed_parameters, exercise=TRUE}
seq(by = ___, ___, ___)
```

```{r pass_mixed_parameters-solution}
seq(by = 2, 0, 10)
```

```{r pass_mixed_parameters-code-check}
grade_code()
```
...
```{r fixed_length_seq, exercise=TRUE}

```

```{r fixed_length_seq-solution}
seq(15.23, 30.24, length.out = 10)
```

```{r fixed_length_seq-code-check}
grade_code()
```
...
```{r rep_a_value, exercise=TRUE}

```

```{r rep_a_value-solution}
rep(10, 20)
```

```{r rep_a_value-code-check}
grade_code()
```
...
```{r rep_and_colon, exercise=TRUE}

```

```{r rep_and_colon-solution}
rep(1:5, 3)
```

```{r rep_and_colon-code-check}
grade_code()
```
...
```{r rep_and_c, exercise=TRUE, exercise.reveal_solution=FALSE}

```

```{r rep_and_c-solution}
rep(c(5,9,2), 4)
```

```{r rep_and_c-code-check}
grade_code()
```
...
```{r sample_letters, exercise=TRUE}

```

```{r sample_letters-solution}
some_letters <- sample(letters, 5)
some_letters
```

```{r sample_letters-code-check}
grade_code()
```
...
```{r sample_normal, exercise=TRUE}

```

```{r sample_normal-solution}
random_normal <- rnorm(10000, mean = 3, sd = 6)
hist(random_normal)
```

```{r sample_normal-code-check}
grade_code()
```
...
```{r every_other_letter, exercise=TRUE}

```

```{r every_other_letter-solution}
letters[seq(2,26, 2)]
```

```{r every_other_letter-code-check}
grade_code()
```
...
```{r index_MSU, exercise=TRUE}
LETTERS[13, 19, 21]
```

```{r index_MSU-solution}
LETTERS[c(13, 19, 21)]
```

```{r index_MSU-code-check}
grade_code()
```
...
```{r reverse_letters, exercise=TRUE}

```

```{r reverse_letters-solution}
letters[26:1]
```

```{r reverse_letters-code-check}
grade_code()
```
...
```{r is_less_than_half, exercise=TRUE}
random_values <- runif(5)
random_values
```

```{r is_less_than_half-solution}
random_values <- runif(5)
random_values
...
```{r is_less_than_half-code-check}
grade_code()
```
...
```{r letter_order, exercise=TRUE}

```

```{r letter_order-solution}
letter_order <- 1:26
letter_order
...
```{r student_grades, exercise=TRUE}
student_grades <- c(89.2, 70.0, 88.2, 99.6, 62.1)
```

```{r student_grades-solution}
student_grades <- c(89.2, 70.0, 88.2, 99.6, 62.1)
...
```{r student_grades_index-setup}
student_grades <- c(89.2, 70, 88.2, 99.6, 62.1)
student_names <- c("Anna", "Bently", "Chloe", "Douglas", "Ellen")
names(student_grades) <- student_names
```

```{r student_grades_index, exercise=TRUE}

```

```{r student_grades_index-solution}
grade_bently <- student_grades["Bently"]
grade_bently
```

```{r student_grades_index-code-check}
grade_code()
```
...
```{r grades_index_lower_case-setup}
student_grades <- c(89.2, 70, 88.2, 99.6, 62.1)
student_names <- c("Anna", "Bently", "Chloe", "Douglas", "Ellen")
names(student_grades) <- student_names
```

```{r grades_index_lower_case, exercise=TRUE}

```

```{r grades_index_lower_case-solution}
student_grades["bentley"]
```

```{r grades_index_lower_case-code-check}
grade_code()
```
...
```{r grades_index_two_names-setup}
student_grades <- c(89.2, 70, 88.2, 99.6, 62.1)
student_names <- c("Anna", "Bently", "Chloe", "Douglas", "Ellen")
names(student_grades) <- student_names
```

```{r grades_index_two_names, exercise=TRUE}

```

```{r grades_index_two_names-solution}
student_grades[c("Chloe", "Ellen")]
```

```{r grades_index_two_names-code-check}
grade_code()
```
...
```{r more_student_names, exercise=TRUE}

```

```{r more_student_names-solution}
more_student_grades
more_student_names <- names(more_student_grades)
more_student_names
```

```{r more_student_names-code-check}
grade_code()
```
...
```{r more_student_grades_class-setup}
dummy <- set.seed(330330)
more_student_grades <- structure(runif(50) * 30 + 70, names = c("Jaylan", 
    "Luke", "Haiden", "Rowan", "Tessa", "Kash", "Valentin", "Lailah", 
    "Amare", "Frankie", "Jude", "Maci", "Abigail", "Sawyer", 
    "Lexi", "Reyna", "Elizabeth", "Rylee", "Leroy", "Camren", 
    "Jordyn", "Kaleigh", "Shyann", "Heaven", "Holden", "Alaina", 
    "Barbara", "Miles", "Isaias", "Carsen", "Tristian", "Chaz", 
    "Chaim", "Felix", "Leilani", "Kyan", "Ashlyn", "Skylar", 
    "Judah", "Meredith", "Deangelo", "Dania", "Abdullah", "Aron", 
    "Julianna", "Todd", "Carolyn", "Desmond", "Gabriela", "Lillianna"))
more_student_names <- names(more_student_grades)
```

```{r more_student_grades_class, exercise=TRUE}

```

```{r more_student_grades_class-solution}
class(more_student_grades)
```

```{r more_student_grades_class-code-check}
grade_code()
```
...
```{r more_student_names_class-setup}
dummy <- set.seed(330330)
more_student_grades <- structure(runif(50) * 30 + 70, names = c("Jaylan", 
    "Luke", "Haiden", "Rowan", "Tessa", "Kash", "Valentin", "Lailah", 
    "Amare", "Frankie", "Jude", "Maci", "Abigail", "Sawyer", 
...
```{r more_student_names_class, exercise=TRUE}

```

```{r more_student_names_class-solution}
class(more_student_names)
```

```{r more_student_names_class-code-check}
grade_code()
```
...
```{r names_before_e, exercise=TRUE}

```

```{r names_before_e-solution}
names(more_student_grades) < "E"
```

```{r names_before_e-code-check}
grade_code()
```
...
```{r grades_for_A_to_M, exercise=TRUE}

```

```{r grades_for_A_to_M-solution}
my_student_grades <- more_student_grades[names(more_student_grades) < "N"]
mean(my_student_grades)
```

```{r grades_for_A_to_M-code-check}
grade_code()
```
...
```{r vector_math_1, exercise=TRUE}

```

```{r vector_math_1-solution}
one_to_ten_a <- 1:10
one_to_ten_b <- 1:10
one_to_ten_a + one_to_ten_b
```

```{r vector_math_1-code-check}
grade_code()
```
...
```{r vector_math_2-setup}
one_to_ten_a <- 1:10
one_to_ten_b <- 1:10
```

```{r vector_math_2, exercise=TRUE}

```

```{r vector_math_2-solution}
one_to_ten_a * one_to_ten_b
one_to_ten_a / one_to_ten_b
one_to_ten_a - one_to_ten_b
one_to_ten_a^one_to_ten_b
...
```{r recycling_1-solution}
one_to_ten_a + c(1,2)
```

```{r recycling_1-code-check}
grade_code()
```
...
```{r recycling_2-solution}
one_to_ten_a + 1:4
```

```{r recycling_2-code-check}
grade_code()
```
